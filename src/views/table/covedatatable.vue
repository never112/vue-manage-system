...
import { fetchData,fetchCodeCovData } from '@/api/index';
import TableCustom from '@/components/table-code.vue';
...
const searchOpt = ref<FormOptionList[]>([
	{ type: 'input', label: '应用名称:', prop: 'appName' }
])
const handleSearch = () => {
	changePage(1);
};
...
let columns = ref([
	...
	{ prop: 'testOwner', label: '测试负责人' },
	{ prop: 'totalLine', label: '代码总行' },
	{ prop: 'updateTime', label: '数据更新时间' },
	{ prop: 'branch', label: '分支', width: 250 },
])
const page = reactive({
	index: 1,
	size: 10,
...
const getData = async () => {
	const res = await fetchCodeCovData()
